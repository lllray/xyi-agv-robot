...
//////////////////////////////////////////////////////////////////////////////////////////////////////
// 暂时放弃.. 应付12月份的避障
static std::vector<kdtree_node> search_path;
...
int kd_tree_finddist_inrange_node(kdtree_t *self_tree,
                                  ...
                                  KEY_TYPE &key)
{
    /// first: search through the data. get the origin dis.
    if(parent==NULL) search_path.clear();
    ...
    else
    {
      ...
      if (key.data()[node->pivot_dim] < node->pivot_value)
        return kd_tree_finddist_inrange_node(self_tree, node, node->children[0], key);
      else
        return kd_tree_finddist_inrange_node(self_tree, node, node->children[1], key);
    }

    // If the keys are the different... set the "node" now as the original nearset node. then search through the search_path.
    static int inrange_dist_min;
    static KEY_TYPE key_dist_cal;
    static std::vector<kdtree_node>::iterator search_it;

    key_dist_cal = node->key - key;
    inrange_dist_min = std::abs(key_dist_cal(0)) + std::abs(key_dist_cal(1));

    /*
    search_it = search_path.begin();
    while(search_it != search_path.end())
    {
        if((*search_it).isleaf == false)
        {
